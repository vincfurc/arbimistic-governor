...
import "@openzeppelin/contracts-upgradeable/governance/extensions/OptimisticGovernorSettingsUpgradeable.sol";
import
    "@openzeppelin/contracts-upgradeable/governance/extensions/GovernorVotesQuorumFractionUpgradeable.sol";
import
    "@openzeppelin/contracts-upgradeable/governance/extensions/GovernorPreventLateQuorumUpgradeable.sol";
import
    "@openzeppelin/contracts-upgradeable/governance/extensions/OptimisticGovernorCountingSimpleUpgradeable.sol";
...
/// @title  ArbimisticGovernor
/// @notice Easy-Track Governance controls for the Arbitrum DAO
/// @dev    Optimistic governor for easy-track proposals.
contract ArbimisticGovernor is
    Initializable,
    OptimisticGovernorSettingsUpgradeable,
    OptimisticGovernorCountingSimpleUpgradeable,
    ...
    OwnableUpgradeable
{
    ...
    uint256 public _lastProposalTimestamp;
    uint256 public _proposalsPerPeriod;
    ...
    /// @param _quorumNumerator The proportion of the circulating supply required to reach an against quorum  
    /// @param _whitelisted The address of the entity authorised to create a proposal
    function initialize(
        ...
        __Governor_init("ArbimisticGovernor");
        __GovernorSettings_init(_votingDelay, _votingPeriod, _whitelisted);
    ...
    function proposalThreshold()
        public
        view
        override(OptimisticGovernorSettingsUpgradeable, GovernorUpgradeable)
        returns (uint256)
    {
        return OptimisticGovernorSettingsUpgradeable.proposalThreshold();
    }
    ...
    function checkMaxActiveProposals() internal {
        if (block.timestamp >= _lastProposalTimestamp + maxProposalsPeriod()) {
            _proposalsPerPeriod = 0;
            _lastProposalTimestamp = block.timestamp;
        }

        // Check if the maximum number of proposals has been reached
        require(_proposalsPerPeriod < maxActiveProposals(), "Governor: max proposals per period exceeded");
